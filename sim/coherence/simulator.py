...
class UniverseSimulator:
    ...
    def generate(
        ...
        for i in range(n):
            ...
            Omega_m = np.clip(
                self.constants.Omega_m * (1 + variation_scale * np.random.randn()),
                0.1, 0.9
            )
    ...
    def statistical_analysis(self, universes: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        params = ['alpha', 'A_s', 'n_s', 'k', 'k_over_alpha', 
    ...
    def find_special_universes(
        ...
        if criterion == 'max_coherence':
            ...
            return [u for u in universes 
                    if abs(u['Omega_m'] + u['Omega_lambda'] - 1) < 0.01]
    ...
    def monte_carlo_k(
        ...
        alpha = np.random.normal(
            ...
            self.constants.alpha_error,
            n_samples
        )
        A_s = np.random.normal(
            self.constants.A_s,
            self.constants.A_s_error,
            n_samples
        )
        n_s = np.random.normal(
            self.constants.n_s,
            self.constants.n_s_error,
            n_samples
        )
        ...
        result = {
            ...
            'observed_in_range': (
                np.percentile(k_samples, 2.5) <= self.constants.k_observed <= 
