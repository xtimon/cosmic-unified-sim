...
class CosmologicalConstants:
    ...
    uncertainties: Dict[str, float] = field(default_factory=lambda: {
        "H0": 0.54,
        "Omega_m": 0.007,
        "Omega_b": 0.0006,
        "Omega_lambda": 0.007,
...
k Formulas Comparison:
  ...
  k (holographic) = {self.k_formula_holographic():.6f} (error: {self.k_error_percent('holographic'):.2f}%)
  k (entropic)    = {self.k_formula_entropic():.6f} (error: {self.k_error_percent('entropic'):.2f}%)
  k (boson mass)  = {self.k_formula_boson_mass():.6f} (error: {self.k_error_percent('boson_mass'):.2f}%)
"""
