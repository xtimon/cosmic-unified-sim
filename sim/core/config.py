...
class SimulationConfig:
        ...
        with open(filepath, 'r') as f:
            if filepath.suffix in ('.yaml', '.yml'):
                if not YAML_AVAILABLE:
                    raise ConfigurationError(
                        "format", "yaml",
                        "PyYAML not installed. Use 'pip install pyyaml'"
                    )
    ...
    def validate(self) -> bool:
        ...
        if self.quantum.num_qubits < 1:
            raise ConfigurationError(
                "quantum.num_qubits", self.quantum.num_qubits, "Must be >= 1"
            )
        ...
        if self.cosmic.integrator not in valid_integrators:
            raise ConfigurationError(
                "cosmic.integrator",
                self.cosmic.integrator,
                f"Must be one of {valid_integrators}"
            )
